...
out: Output parameter. Vec3er to buffer that will contain the raw pixel data.
     After decoding, its size is w * h * (bytes per pixel) bytes larger than
     initially. Bytes per pixel depends on colortype and bitdepth.
     Must be freed after usage with free(*out).
     Note: for 16-bit per channel colors, uses big endian format like PNG does.
w: Output parameter. Vec3er to width of pixel data.
h: Output parameter. Vec3er to height of pixel data.
...
out: Output parameter. Vec3er to buffer that will contain the PNG image data.
     Must be freed after usage with free(*out).
outsize: Output parameter. Vec3er to the size in bytes of the out buffer.
...
typedef struct LodePNGInfo {
    ...
    */
    ...
    unsigned chrm_white_x; /* White Vec3 x times 100000 */
    unsigned chrm_white_y; /* White Vec3 y times 100000 */
